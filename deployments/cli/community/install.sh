...
export IMAGE_REGISTRY=ghcr.io/abuhtoyarov
export PULL_POLICY=${PULL_POLICY:-if_not_present}
export GH_REPO=abuhtoyarov/lokso-app
...
function initialize(){
    ...
    local IMAGE_NAME="${IMAGE_REGISTRY}/lokso-proxy"
    ...
    if [ $? -eq 0 ]; then
        echo "Lokso supports ${CPU_ARCH}" >&2
        echo "available"
        return 0
    else
        echo "" >&2
...
function updateCustomVariables(){
    ...
    updateEnvFile "IMAGE_REGISTRY" "$IMAGE_REGISTRY" "$DOCKER_ENV_PATH"
...
function download() {
    ...
    echo ""
    echo "Most recent version of Lokso is now available for you to use"
    echo ""
...
function startServices() {
    ...
    if [ -n "$migrator_container_id" ]; then
        ...
        if [ $migrator_exit_code -ne 0 ]; then
            echo "Lokso Server failed to start ❌"
            # stopServices
            echo
    ...
    echo "   Lokso Server started successfully ✅"
    echo ""
...
function upgrade() {
    ...
    echo "Upgrading Lokso to the latest release..."
    echo ""
...
if [ -f "$DOCKER_ENV_PATH" ]; then
    IMAGE_REGISTRY=$(getEnvValue "IMAGE_REGISTRY" "$DOCKER_ENV_PATH")
    ...
    if [ -z "$IMAGE_REGISTRY" ]; then
        IMAGE_REGISTRY=ghcr.io/abuhtoyarov
        updateEnvFile "IMAGE_REGISTRY" "$IMAGE_REGISTRY" "$DOCKER_ENV_PATH"
    fi
