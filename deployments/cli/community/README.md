...
You have successfully self hosted `Lokso` instance. Access the application by going to IP or domain you have configured it (e.g `https://plane.example.com:8080` or `http://[IP-ADDRESS]:8080`)
...
### Upgrading Lokso Version

It is always advised to keep Lokso up to date with the latest release.
...
1. Make sure that Lokso is installed, started, and then stopped. This ensures that the Docker volumes are created.
...
1. Start the Lokso instance using `./setup.sh start`.
