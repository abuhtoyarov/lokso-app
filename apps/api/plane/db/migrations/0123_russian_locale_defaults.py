...
def apply_russian_defaults(apps, schema_editor):
    ...
    Only rows holding the previous default are touched. Someone who picked
    English deliberately keeps it — that cannot be told apart from the default
    having been written for them, so the narrower rule is the safer one.
    """
