...
class VKOauthInitiateSpaceEndpoint(View):
    def get(self, request):
        ...
        try:
            ...
            provider = VKOAuthProvider(request=request, state=state)
            request.session["state"] = state
            # PKCE: keep the code verifier for the token exchange on callback
            request.session["code_verifier"] = provider.code_verifier
...
class VKCallbackSpaceEndpoint(View):
    def get(self, request):
